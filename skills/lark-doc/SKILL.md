...
name: lark-doc
version: 1.0.0
description: "飞书云文档：创建/编辑文档（Markdown→doc），获取文档内容，更新文档（追加/覆盖/替换/插入/删除），上传下载图片文件，搜索云空间文档。触发：文档, doc, 写文档, 创建文档, 编辑文档. 也适用于按名称定位表格/报告。不适用于：电子表格（用 lark-sheets）、多维表格（用 lark-base）、知识库节点管理（用 lark-wiki）。"
metadata:
  requires:
    bins: ["lark-cli"]
