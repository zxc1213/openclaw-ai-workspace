...
name: lark-shared
version: 1.0.0
description: "飞书/Lark CLI 共享基础：配置初始化(auth login)、身份切换(--as user/bot)、权限与 scope 管理、Permission denied 错误处理。触发：lark-cli首次配置, 授权, auth login, 权限不足, Permission denied. 不适用于：具体飞书操作（用对应 lark-* skill）、非 lark-cli 工具。"
---
