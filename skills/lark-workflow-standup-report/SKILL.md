...
version: 1.0.0
description: "日程待办摘要：编排 calendar +agenda 和 task +get-my-tasks，生成指定日期的日程与未完成任务摘要。触发：今日安排, 明天安排, 本周安排, standup, 站会. 需要 lark-calendar 和 lark-task。 不适用于：会议纪要汇总（用 lark-workflow-meeting-summary）、详细日程编辑。"
metadata:
  requires:
    bins: ["lark-cli"]
---
