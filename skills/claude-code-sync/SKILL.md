...
| 端 | 配置目录 |
|----|---------|
| Windows | `/mnt/c/Users/example-user/.claude/` |
...
```bash
# 1. CLAUDE.md
cp SOURCE/.claude/CLAUDE.md /mnt/c/Users/example-user/.claude/CLAUDE.md
cp SOURCE/.claude/CLAUDE.md /home/rays/.claude/CLAUDE.md

# 2. Skills（双向合并，源端覆盖目标端同名 skill）
rsync -av --delete SOURCE/.claude/skills/ /mnt/c/Users/example-user/.claude/skills/
rsync -av --delete SOURCE/.claude/skills/ /home/rays/.claude/skills/

# 3. Commands
rsync -av --delete SOURCE/.claude/commands/ /mnt/c/Users/example-user/.claude/commands/
...
```bash
# 两端文件数对比
diff <(ls /mnt/c/Users/example-user/.claude/skills/) <(ls /home/rays/.claude/skills/)
diff <(ls /mnt/c/Users/example-user/.claude/commands/) <(ls /home/rays/.claude/commands/)
```
...
mkdir -p "$BACKUP_DIR"
cp -r /mnt/c/Users/example-user/.claude/{CLAUDE.md,skills,commands,settings.json} "$BACKUP_DIR/windows/"
cp -r /home/rays/.claude/{CLAUDE.md,skills,commands,settings.json} "$BACKUP_DIR/wsl/"
```

## 飞书版本追踪

知识空间 **Claude Code 配置管理**（Space ID: 7600000000000000000）中维护版本清单和变更记录。
