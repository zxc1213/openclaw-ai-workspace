---
name: ontology
description: "类型化知识图谱，用于 Agent 结构化记忆和可组合技能。支持创建/查询实体（Person/Project/Task/Event/Document）、关联对象、约束执行、将多步操作规划为图变换。触发：记住、remember、我知道什么关于、关联 X 和 Y、依赖、实体 CRUD、跨技能数据访问。需要 ontology 后端配置。不适用于：简单键值笔记（用 memory 工具）、临时任务状态（用 clawflow）。"
---
