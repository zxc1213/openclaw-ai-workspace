...
name: lark-mail
version: 1.0.0
description: "飞书邮箱：起草、撰写、发送、回复、转发、读取、搜索邮件；管理草稿、文件夹、标签、联系人、附件。 触发：起草邮件, 写邮件, 发邮件, 回复邮件, 转发邮件, 查看邮件, 收件箱, 邮件会话, mail, email. 需要 lark-cli mail 权限。 不适用于：日历邀请（用 lark-calendar）、即时消息（用 lark-im）。"
metadata:
  requires:
    bins: ["lark-cli"]
