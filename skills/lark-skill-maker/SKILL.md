...
version: 1.0.0
description: "创建 lark-cli 自定义 Skill — 封装飞书 API 操作为可复用的 Skill（原子 API 或多步流程编排）。触发：创建技能, 封装skill, lark skill, 自定义skill. 需要 lark-cli 已配置。 不适用于：安装/编辑已有技能（用 find-skills/skill-creator）、非飞书技能创建。"
metadata:
  requires:
    bins: ["lark-cli"]
---
