...
version: 1.0.0
description: "会议纪要整理工作流：汇总指定时间范围内的会议纪要并生成结构化报告。触发：整理会议纪要, 会议周报, 回顾会议, meeting summary. 需要 lark-vc 和 lark-calendar。 不适用于：单次会议纪要（直接用 lark-vc）、每日站会（用 lark-workflow-standup-report）。"
metadata:
  requires:
    bins: ["lark-cli"]
---
