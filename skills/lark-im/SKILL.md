...
name: lark-im
version: 1.0.0
description: "飞书即时通讯：收发消息、搜索聊天记录、管理群聊成员、上传下载图片文件、管理表情回复。触发：发消息, 聊天记录, 搜索消息, 群聊, IM, message. 不适用于：读取当前对话中的 IM 消息附件（用 feishu_im_bot_image）、代用户发消息（用 feishu_im_user_message）。"
metadata:
  requires:
    bins: ["lark-cli"]
