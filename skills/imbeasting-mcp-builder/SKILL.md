---
name: mcp-builder
description: "MCP Server 开发指南：创建高质量 MCP 服务器，让 LLM 通过精心设计的工具与外部服务交互。支持 Python（FastMCP）和 Node/TypeScript（MCP SDK）。触发：创建 MCP、build MCP server、MCP 开发。不适用于：消费已有 MCP 服务器、非 MCP 工具集成。"
