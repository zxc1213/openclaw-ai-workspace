...
name: lark-vc
version: 1.0.0
description: "飞书视频会议：查询已结束的会议记录、获取会议纪要产物（总结、待办、章节、逐字稿）。触发：会议记录, 会议纪要, VC, 视频会议, meeting notes. 未来会议请用 lark-calendar。 支持按关键词、时间、组织者、参与者、会议室筛选。 不适用于：安排会议（用 lark-calendar）、实时会议控制。"
metadata:
  requires:
    bins: ["lark-cli"]
