...
1. **必须用绝对路径** — subagent workspace 与主会话不同，相对路径会找不到项目
2. **`mode=session` 需要飞书/Discord等 channel 支持 thread=true**，不支持时会静默失败
3. **timeout 必须显式设置** — 简单600s，中等1200s，复杂1800s，不要依赖默认值
4. **git 操作在主会话完成** — 分支创建/reset/fetch 交给 subagent 会因路径问题失败
5. **任务描述精简再精简** — 只给必要文件路径和需求，不要塞整个项目上下文
