...
name: lark-base
version: 1.2.0
description: "飞书多维表格（Base）操作：建表、字段管理、记录读写、视图配置、历史查询、角色/表单/仪表盘管理。触发：多维表格, bitable, 数据表, +table, +field, +record, 数据分析. 需要 lark-cli 已配置。 不适用于：电子表格/Excel 操作（用 lark-sheets）、简单文件存储（用 lark-drive）。"
metadata:
  requires:
    bins: ["lark-cli"]
